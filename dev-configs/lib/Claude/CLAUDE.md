...
6. Assignees: `hayat01sh1da` and `claude[bot]`(if assignable).
...
### 1-3. Daily Dependencies Update Workflows

The `*--daily-dependencies-update.yml` workflows (30 of them across 14 repositories) open their pull requests with `peter-evans/create-pull-request@v8`, authored by `github-actions[bot]`, with `reviewers: hayat01sh1da` and `assignees: hayat01sh1da`.

- **Decision (2026-09-07): keep that shape as it is.** The identity is not traded away to silence the CI approval prompt — `github-actions[bot]` stays the author and `hayat01sh1da` stays the assignee and requested reviewer, and the approval click on each daily pull request is accepted as the cost. A cross-repository change that switched the action to a personal access token was prepared and then abandoned for this reason (closed on 2026-09-07: [coding-tests#211](https://github.com/hayat01sh1da/coding-tests/pull/211), [spreen-wiki#159](https://github.com/hayat01sh1da/spreen-wiki/pull/159) and 12 more, one per repository). Do not re-propose it without a new reason.
- **Why the prompt exists** (still true, so it does not need rediscovering): since [2026-06-11](https://github.blog/changelog/2026-06-11-bot-created-pull-requests-can-run-workflows-if-approved/) GitHub holds every workflow run of a pull request that `github-actions[bot]` creates *or updates* in the `action_required` state until a user with write access approves it — same-repository branches included, not just forks. Attempt 1 of such a run has `conclusion: action_required`; the approved re-run shows up as attempt 2.
- **No setting removes it.** `actions/permissions/fork-pr-contributor-approval` accepts only `first_time_contributors_new_to_github`, `first_time_contributors` and `all_external_contributors`, all of which describe fork contributors rather than bots; `can_approve_pull_request_reviews` governs whether Actions may submit pull request reviews, which is unrelated. The gate keys on the identity that opens the pull request, so only replacing that identity removes it — with a personal access token (author becomes `hayat01sh1da`, which forfeits the `reviewers` input and self-approval) or a GitHub App installation token (author becomes `<app>[bot]`, which keeps both, at the cost of an App plus `APP_ID` and a private key per repository). Approving the held runs through `POST /actions/runs/{run_id}/approve` is not a shortcut: it needs a token with `actions: write` of its own, and is documented for fork pull requests.
- **`reviewers: hayat01sh1da` is only valid while the bot is the author.** If the identity ever does change to `hayat01sh1da`, that input has to go in the same commit: GitHub answers `Review cannot be requested from pull request author`, and the action rethrows that error (`src/github-helper.ts` downgrades only the collaborator-scope error to a warning), failing the workflow after the pull request has already been opened.
