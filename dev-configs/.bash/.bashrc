...
## Rubbish Cleanup
# Deletes desktop.ini files, tool caches and temp directories under $HOME, then
# prunes the directories left empty. No sudo and no "cd /": it stays on one
# filesystem and never walks system paths, repositories or dependency trees.
alias rmra='find "${HOME:?}" -xdev -regextype posix-extended \( -name .git -o -name node_modules -o -name .venv -o -name venv \) -prune -o \( -type f -name desktop.ini -o -type d -regex ".*/(__pycache__|[.][a-z]+_cache|[Tt]e?mp)" \) -prune -print -exec rm -rf {} + ; find "${HOME:?}" -xdev -mindepth 2 -type d -empty -not -path "*/.git/*" -not -path "*/node_modules/*" -not -path "*/.venv/*" -print -delete && cd -'
...
alias rmd="find . -type f -name 'desktop.ini' -exec rm -f {} +"

## Python Cache
# The name is matched against the whole path, so it needs the leading ".*/".
alias lsp="find . -type d -regextype posix-extended -regex '.*/(__pycache__|[.][a-z]+_cache)' -print"
alias rmp="find . -type d -regextype posix-extended -regex '.*/(__pycache__|[.][a-z]+_cache)' -prune -exec rm -rf {} +"

## Temporary Files
alias lst="find . -type d -regextype posix-extended -regex '.*/[Tt]e?mp' -print"
alias rmt="find . -type d -regextype posix-extended -regex '.*/[Tt]e?mp' -prune -exec rm -rf {} +"
...
alias grurl="git remote get-url"
...
# Single-quoted so the secret is read at call time instead of being baked into
# the alias body at load time (where `alias` would print it in the clear).
alias cact='printf %s "$GITHUB_ACCESS_TOKEN" | clip.exe && echo "GITHUB_ACCESS_TOKEN copied to clipboard"'
alias crsa='printf %s "$RSA_PASSWORD" | clip.exe && echo "RSA password copied to clipboard"'
...
alias cpclnp="cp ~/Home/Development/github-profile/dev-configs/scripts/clone_personal_repos.sh ~/Home/Development/personal/"
alias cpfudp="cp ~/Home/Development/github-profile/dev-configs/scripts/flush_and_update_personal_repos.sh ~/Home/Development/personal/"
...
### OSS Repositories
alias cpclno="cp ~/Home/Development/github-profile/dev-configs/scripts/clone_oss_repos.sh ~/Home/Development/oss/"
alias cpdlo="cp ~/Home/Development/github-profile/dev-configs/scripts/delete_oss_remote_repos.sh ~/Home/Development/oss/"
alias cpfudo="cp ~/Home/Development/github-profile/dev-configs/scripts/flush_and_update_oss_repos.sh ~/Home/Development/oss/"
alias clno="bash ~/Home/Development/github-profile/dev-configs/scripts/clone_oss_repos.sh"
alias dlo="bash ~/Home/Development/github-profile/dev-configs/scripts/delete_oss_remote_repos.sh"
alias fudo="bash ~/Home/Development/github-profile/dev-configs/scripts/flush_and_update_oss_repos.sh"

### Shared Repositories
alias cpudp="cp ~/Home/Development/github-profile/dev-configs/scripts/update_package_dependencies.sh ~/Home/Development/personal/"
alias fuda="
...
## rbenv
alias rbesup="git clone git@github.com:rbenv/rbenv.git ~/.rbenv && mkdir -p ~/.rbenv/plugins/ruby-build && cd ~/.rbenv/plugins/ruby-build && git clone git@github.com:rbenv/ruby-build.git . && cd -"
...
alias pistu="pip install --upgrade"
...
## ndenv
alias ndesup="git clone git@github.com:riywo/ndenv.git ~/.ndenv && mkdir -p ~/.ndenv/plugins/node-build && cd ~/.ndenv/plugins/node-build && git clone git@github.com:riywo/node-build.git . && cd -"
...
alias dpnpls="docker-compose exec app pnpm list"
...
# Others
alias chdn='printf %s "$HIDDEN_PASSWORD" | clip.exe && echo "Hidden password copied to clipboard"'
